...
cols = st.columns(3)
with cols[0]:
    ew = input_field("Empty Weight", "ew", 0.0, ac['units']['weight'], helptext="Basic empty mass as per W&B sheet")
    ew_arm = input_field("Empty Weight Arm", "ew_arm", 0.0, ac['units']['arm'], helptext="Arm for empty mass")
with cols[1]:
...
def generate_pdf(aircraft, registration, mission_number, flight_datetime_utc,
    ...
    pdf.set_text_color(25,91,166)
    pdf.cell(0, 8, "Auto-generated by EASA Mass & Balance Planner | For training only | " +
                    f"{datetime.datetime.now(pytz.UTC).strftime('%Y-%m-%d %H:%M UTC')}", ln=True, align='C')
    pdf.set_text_color(0,0,0)
    return pdf
